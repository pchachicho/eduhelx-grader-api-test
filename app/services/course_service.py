...
class CourseService:
    ...
    async def create_course(self, name: str, start_at: datetime = None, end_at: datetime = None) -> CourseModel:
        from app.services import GiteaService, FileOperation, FileOperationType, CleanupService
        ...
        staging_repository_name = self._compute_staging_repository_name(name)
        ...
        gitignore_file = [FileOperation(content=".ssh", path=".gitignore", operation=FileOperationType.CREATE)]
        ...
        try:
            ...
            staging_remote_url = await gitea_service.create_repository(
                name=staging_repository_name,
                description=f"The class staging repository for { name }",
                owner=instructor_organization_name,
                private=True
            )
            ...
            course.staging_remote_url = staging_remote_url
            self.session.commit()
    ...
    async def update_course(self, update_course: UpdateCourseSchema) -> CourseModel:
        ...
        if "staging_remote_url" in update_fields:
            course.staging_remote_url = update_fields["staging_remote_url"]
    ...
    async def get_staging_repository_name(self) -> str:
        course = await self.get_course()
        return self._compute_staging_repository_name(course.name)
    ...
    @staticmethod
    def _compute_staging_repository_name(course_name: str) -> str:
        return f"{ course_name.replace(' ', '_') }-class-staging-repo"
